  ...
  public size(srcUri: string): Promise<number> {
    return new Promise((resolve, reject) => {
      request.head(srcUri, { followAllRedirects: true, followOriginalHttpMethod: true }, (err, res) => {
        ...
        // 상태 코드 확인
        if (res.statusCode !== 200) return reject(new Error(`Cannot get size of remote file (code: ${res.statusCode}) : ${srcUri}`));

        // 헤더 추출
        const contentLength = parseInt(res.caseless.get('content-length'));
        if (isNaN(contentLength)) return reject(new Error(`Cannot get size of remote file (invalid content-length header) : ${srcUri}`));
